...
% The abstract must be a Structured Abstract with the headings {\bf Context/Background}, {\bf Aims}, {\bf Method}, {\bf Results}, and {\bf Conclusions}.  This section should not be longer than half of a page, and having no more than one or two sentences under each heading is advised.
	...
	{\bf Method -- }
	A number of open-source datasets will be collected, and the state of the art for each dataset identified if used in past research. A number of \textit{Machine Learning} models will be implemented (\textit{SVM, Naive Bayes, Gradient Boosted Classifier} etc.) to form a baseline of performance which provides motivation to improve these results with \textit{Deep Learning} methods (\textit{RNNs, CNNs,} etc). The improvement in performance (if any) will be assessed. Furthermore, cutting-edge methods such as \textit{ELMo} word representations and new ideas such as custom loss functions will be evaluated along the way.
	
	{\bf Results -- }
	High performance is achieved - above 0.76 F1 score on all four datasets used in this project, including 0.833 on one Twitter dataset. For the binary problem (detecting if cyberbullying is present or not), \textit{Deep Learning} methods out-performed traditional \textit{Machine Learning} methods by approximately 3\%, however no improvement was seen on the 3-class problem (racism, sexism or neither). The extra time and resources required to implement such complex \textit{Deep Learning} models (such as using \textit{ELMo} word representations) must be considered for such marginal performance improvements on this task.
	
	{\bf Conclusions -- }
	The solution detects cyberbullying with rather high performance which renders the project a success, yet there is definitely room for improvement. A good underlying dataset is vital for success in this task, and the publicly available datasets are too small, most likely due to the offensive nature of the data which provides a deterrent for anyone wanting to gather such data. One might want to gather as much open-source data as possible, and combine these small datasets into one large corpus.
\end{abstract}
